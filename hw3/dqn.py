...
def learn(env,
    ...
    q_target_values = q_func(obs_tp1_float, num_actions, 'target_q_func', reuse=False)
    target_q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='target_q_func')

    eval_acts = tf.one_hot(act_t_ph, num_actions, dtype=tf.float32)
    delta_t = rew_t_ph + (gamma * tf.reduce_max(q_target_values, axis=1) - tf.reduce_sum(eval_acts * q_eval_values, axis=1))
    ...
    for t in itertools.count():
        ...
        if not model_initialized:
            ...
            q = session.run(q_eval_values, feed_dict={obs_t_ph: img_in[None, :]})
            action = np.argmax(q)
        next_state, reward, done, _ = env.step(action)
        replay_buffer.store_effect(frame_id, action=action, done=done, reward=reward)
        ...
        if (t > learning_starts and
            ...
            batch_state, batch_action, batch_reward, batch_next, batch_done = replay_buffer.sample(batch_size)
            if not model_initialized:
